...
app.post("/analytics/add", async (req, res) => {
  try {
    const analytics = await Analytics.findOneAndUpdate({}, { $inc: { addCount: 1 } }, { new: true, upsert: true });
...
app.post("/analytics/clear", async (req, res) => {
  try {
    const analytics = await Analytics.findOneAndUpdate({}, { $inc: { clearCount: 1 } }, { new: true, upsert: true });
